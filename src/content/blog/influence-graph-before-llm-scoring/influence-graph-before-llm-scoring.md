...
publishDate: '2026-07-16 20:30:00'
tags:
  - AI Agent
  - LLM
  - Python
...
因此，当前更准确的成熟度判断是：**文本到影响图、参考节点约束和候选边回源验证已经实现；端到端证据血缘、失败状态建模和评分有效性验证仍需要继续补齐。**

# 八、写在最后

直接让 LLM 打分，最省事的是没有中间过程；最让我睡不踏实的，也恰恰是没有中间过程。分数是出来了，可我说不清它从哪来，错了也不知道该从哪下手改。

所以我宁愿多花一层功夫去做这道“编译”：先把一场吵吵嚷嚷的讨论压成节点、关系和“谁提到了什么”的记录，再让图算法、规则、Embedding、统计模型和 LLM 各做各擅长的那部分。这层中间表示没让评分变得天然正确——它还有一堆我自己都清楚的坑——但至少，错误第一次有了落脚的地方：我能指着某个节点、某条边说一句“就是这里”，然后去改。

写到这儿我才想明白，让我踏实的其实不是模型有多强，而是当有人追问“凭什么”的时候，我手里有东西能摊开给他看。这大概是我做这一版时，最想守住的一点。
